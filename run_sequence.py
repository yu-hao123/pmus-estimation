...
def main():
    ...
    if args.path.name.startswith("ASL_spont_"):
        ins_marks, exp_marks = retrieve_parity_marks(data["volume"].to_numpy() * 10)
    else:
        ins_marks, exp_marks = retrieve_flow_marks(data["flow"].to_numpy(), fs)
    ...
    if args.cycles is not None:
        ...
        start, stop = (int(v) for v in args.slice.split(":"))
        cycle_indices = []
        for k in range(n_cycles):
            cycle_start = int(ins_marks[k]) - args.offset
            cycle_stop = int(ins_marks[k + 1]) - args.offset
    ...
    for k in cycle_indices:
        ...
        pmus_hat, R_hat, E_hat, _, solver_time = pmus_miqp_full(
            cycle, tau_soe=args.tau,
        )
        ...
        cycle_boundaries.append(cycle_boundaries[-1] + cycle.pressure.size)
    ...
    for ax in axes:
        for b in cycle_boundaries[1:-1]:
            ax.axvline(t[b], color="tab:gray", linestyle=":", linewidth=0.8)

    fig.suptitle(
        f"{args.path.name} cycles, "
        f"#{cycle_indices[0]}..{cycle_indices[-1]}"
    )
    fig.tight_layout()
    plt.show()
